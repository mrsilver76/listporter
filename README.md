...
You can find out where the logs are located by reading the output of `PlexPU -h`. There is a hidden command line argument `-v` or `--verbose` which will also log the API calls made to the Plex server. Logs older than 14 days are deleted the next time PlexPU runs.
